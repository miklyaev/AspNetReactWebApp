...
| Priority | TaskPriority | Приоритет: ToDo, InProgress, Done, Canceled |
| PlannedTime | double | Планируемое время на задачу (часы) |
| TimeSpent | double | Суммарное затраченное время (часы) || ProjectId | int | Внешний ключ → Project |
...
#### TimeEntry (Запись учёта времени) — JiraClone.Data/Domain/Entities/TimeEntry.cs - в данный момент не используется
| Поле | Тип | Описание |
|------|-----|----------|
...
## Первоначальная настройка
 ...
 Администратор может добавить ответственных лиц и исполнителей задач. И вообще он имеет полный доступ ко всем возможностям приложения.
